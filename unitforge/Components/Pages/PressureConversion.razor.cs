...
public partial class PressureConversion : ComponentBase
{
    [Inject]
    public IPressureConversionService? PressureConversionService { get; set; }

    private readonly List<string> _fromUnits = [..UnitCategories.Categories["Pressure"]];
    private readonly List<string> _toUnits = [..UnitCategories.Categories["Pressure"]];
    ...
    private void Recalculate()
    {
        ...
        _outputValue = double.TryParse(_inputValue, out var inputValue) ? 
            PressureConversionService!.Convert(inputValue, _fromUnit, _toUnit).ToString(CultureInfo.InvariantCulture) : 
            "Invalid input";
    }
