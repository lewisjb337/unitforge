...
public partial class WeightConversion : ComponentBase
{
    [Inject]
    public IWeightConversionService? WeightConversionService { get; set; }

    private readonly List<string> _fromUnits = [..UnitCategories.Categories["Weight"]];
    private readonly List<string> _toUnits = [..UnitCategories.Categories["Weight"]];
    ...
    private void Recalculate()
    {
        ...
        _outputValue = double.TryParse(_inputValue, out var inputValue) ? 
            WeightConversionService!.Convert(inputValue, _fromUnit, _toUnit).ToString(CultureInfo.InvariantCulture) : 
            "Invalid input";
    }
