...
public partial class FuelConsumptionConversion : ComponentBase
{
    [Inject]
    public IFuelConsumptionConversionService? FuelConsumptionConversionService { get; set; }

    private readonly List<string> _fromUnits = [..UnitCategories.Categories["Fuel"]];
    private readonly List<string> _toUnits = [..UnitCategories.Categories["Fuel"]];
    ...
    private void Recalculate()
    {
        ...
        _outputValue = double.TryParse(_inputValue, out var inputValue) ? 
            FuelConsumptionConversionService!.Convert(inputValue, _fromUnit, _toUnit).ToString(CultureInfo.InvariantCulture) : 
            "Invalid input";
    }
