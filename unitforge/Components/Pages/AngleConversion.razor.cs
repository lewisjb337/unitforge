...
public partial class AngleConversion : ComponentBase
{
    [Inject]
    public IAngleConversionService? AngleConversionService { get; set; }

    private readonly List<string> _fromUnits = [..UnitCategories.Categories["Angle"]];
    private readonly List<string> _toUnits = [..UnitCategories.Categories["Angle"]];
    ...
    private void Recalculate()
    {
        ...
        _outputValue = double.TryParse(_inputValue, out var inputValue) ? 
            AngleConversionService!.Convert(inputValue, _fromUnit, _toUnit).ToString(CultureInfo.InvariantCulture) : 
            "Invalid input";
    }
